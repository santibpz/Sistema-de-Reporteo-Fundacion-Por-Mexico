...
const ComentarioBox = ({
  ...
  const deleteComment = () => {
    ...
    confirm(
      "¿Estás seguro que quieres borrar el comentario? \n Esta acción es irreversible."
    )
      ? deleteOne(
          "comentarios",
          { id },
          {
            onSuccess: () => {
              refetchComentarios();
              notify("El comentario se ha borrado con éxito", {
                type: "success",
              });
            },
            onError: () => {
              notify(
                "Ha ocurrido un error al intentar borrar el comentario. Intentar más tarde.",
                { type: "error" }
              );
            },
          }
